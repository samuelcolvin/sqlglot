...
class TestDataframeFunc(DataFrameSchemaValidator):
    def test_simple_select(self):
        df_employee = self.df_employee.select(F.col("employee_id"))
        dfs_employee = self.dfs_employee.select(SF.col("employee_id"))
    ...
    def test_simple_select_df_attribute(self):
        df_employee = self.df_employee.select(self.df_employee.employee_id)
        dfs_employee = self.dfs_employee.select(self.dfs_employee.employee_id)
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)
    ...
    def test_multiple_selects(self):
        df_employee = self.df_employee.select(
            self.df_employee["employee_id"], F.col("fname"), self.df_employee.lname
        )
        dfs_employee = self.dfs_employee.select(
            self.dfs_employee["employee_id"], SF.col("fname"), self.dfs_employee.lname
        )
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_alias_no_op(self):
        df_employee = self.df_employee.alias("df_employee")
        dfs_employee = self.dfs_employee.alias("dfs_employee")
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_alias_with_select(self):
        df_employee = self.df_employee.alias("df_employee").select(
            self.df_employee["employee_id"],
            F.col("df_employee.fname"),
            self.df_employee.lname,
        )
        dfs_employee = self.dfs_employee.alias("dfs_employee").select(
            self.dfs_employee["employee_id"],
            SF.col("dfs_employee.fname"),
            self.dfs_employee.lname,
        )
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_case_when_otherwise(self):
        df = self.df_employee.select(
            F.when(
    ...
    def test_where_clause_single(self):
        df_employee = self.df_employee.where(F.col("age") == F.lit(37))
        dfs_employee = self.dfs_employee.where(SF.col("age") == SF.lit(37))
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_where_clause_multiple_and(self):
        df_employee = self.df_employee.where(
            (F.col("age") == F.lit(37)) & (F.col("fname") == F.lit("Jack"))
        )
        dfs_employee = self.dfs_employee.where(
    ...
    def test_where_multiple_chained(self):
        df_employee = self.df_employee.where(F.col("age") == F.lit(37)).where(
            self.df_employee.fname == F.lit("Jack")
        )
        dfs_employee = self.dfs_employee.where(SF.col("age") == SF.lit(37)).where(
            self.dfs_employee.fname == SF.lit("Jack")
        )
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_operators(self):
        df_employee = self.df_employee.where(self.df_employee["age"] < F.lit(50))
        dfs_employee = self.dfs_employee.where(self.dfs_employee["age"] < SF.lit(50))
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

        df_employee = self.df_employee.where(self.df_employee["age"] <= F.lit(37))
        dfs_employee = self.dfs_employee.where(self.dfs_employee["age"] <= SF.lit(37))
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)
        ...
        df_employee = self.df_employee.where(self.df_employee["age"] % F.lit(5) == F.lit(0))
        dfs_employee = self.dfs_employee.where(self.dfs_employee["age"] % SF.lit(5) == SF.lit(0))
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

        df_employee = self.df_employee.where(self.df_employee["age"] + F.lit(5) > F.lit(28))
        dfs_employee = self.dfs_employee.where(self.dfs_employee["age"] + SF.lit(5) > SF.lit(28))
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)
        ...
        df_employee = self.df_employee.where(
            self.df_employee["age"] * F.lit(0.5) == self.df_employee["age"] / F.lit(2)
        )
        dfs_employee = self.dfs_employee.where(
            self.dfs_employee["age"] * SF.lit(0.5) == self.dfs_employee["age"] / SF.lit(2)
        )
        self.compare_spark_with_sqlglot(df_employee, dfs_employee)

    def test_join_inner(self):
        df_joined = self.df_employee.join(self.df_store, on=["store_id"], how="inner").select(
            self.df_employee.employee_id,
            ...
            self.df_store["num_sales"],
        )
        dfs_joined = self.dfs_employee.join(self.dfs_store, on=["store_id"], how="inner").select(
            self.dfs_employee.employee_id,
            ...
            self.dfs_store["num_sales"],
        )
        self.compare_spark_with_sqlglot(df_joined, dfs_joined)

    def test_join_inner_no_select(self):
        df_joined = self.df_employee.select(F.col("store_id"), F.col("fname"), F.col("lname")).join(
            self.df_store.select(F.col("store_id"), F.col("store_name")),
            on=["store_id"],
            how="inner",
        )
        dfs_joined = self.dfs_employee.select(
            SF.col("store_id"), SF.col("fname"), SF.col("lname")
        ).join(
            self.dfs_store.select(SF.col("store_id"), SF.col("store_name")),
            on=["store_id"],
            how="inner",
        )
        self.compare_spark_with_sqlglot(df_joined, dfs_joined)
    ...
    def test_join_inner_equality_single(self):
        df_joined = self.df_employee.join(
            self.df_store,
            on=self.df_employee.store_id == self.df_store.store_id,
            how="inner",
        ).select(
            self.df_employee.employee_id,
            self.df_employee["fname"],
            F.col("lname"),
            F.col("age"),
            self.df_employee.store_id,
            self.df_store.store_name,
            self.df_store["num_sales"],
            F.lit("literal_value"),
        )
        dfs_joined = self.dfs_employee.join(
            self.dfs_store,
            on=self.dfs_employee.store_id == self.dfs_store.store_id,
            how="inner",
        ).select(
            self.dfs_employee.employee_id,
            self.dfs_employee["fname"],
    ...
    def test_join_inner_equality_multiple(self):
        df_joined = self.df_employee.join(
            self.df_store,
            on=[
                self.df_employee.store_id == self.df_store.store_id,
                self.df_employee.age == self.df_store.num_sales,
            ],
            how="inner",
        ).select(
            self.df_employee.employee_id,
        ...
        dfs_joined = self.dfs_employee.join(
            self.dfs_store,
            on=[
                self.dfs_employee.store_id == self.dfs_store.store_id,
                self.dfs_employee.age == self.dfs_store.num_sales,
            ],
            how="inner",
        ).select(
            self.dfs_employee.employee_id,
    ...
    def test_join_inner_equality_multiple_bitwise_and(self):
        df_joined = self.df_employee.join(
            self.df_store,
            on=(self.df_store.store_id == self.df_employee.store_id)
            & (self.df_store.num_sales == self.df_employee.age),
            how="inner",
        ).select(
            self.df_employee.employee_id,
            self.df_employee["fname"],
        ...
        dfs_joined = self.dfs_employee.join(
            self.dfs_store,
            on=(self.dfs_store.store_id == self.dfs_employee.store_id)
            & (self.dfs_store.num_sales == self.dfs_employee.age),
            how="inner",
        ).select(
            self.dfs_employee.employee_id,
            self.dfs_employee["fname"],
    ...
    def test_join_left_outer(self):
        df_joined = (
            self.df_employee.join(self.df_store, on=["store_id"], how="left_outer")
            .select(
                self.df_employee.employee_id,
                self.df_employee["fname"],
                F.col("lname"),
        ...
        dfs_joined = (
            self.dfs_employee.join(self.dfs_store, on=["store_id"], how="left_outer")
            .select(
                self.dfs_employee.employee_id,
                self.dfs_employee["fname"],
                SF.col("lname"),
    ...
    def test_join_full_outer(self):
        df_joined = self.df_employee.join(self.df_store, on=["store_id"], how="full_outer").select(
            self.df_employee.employee_id,
            self.df_employee["fname"],
            F.col("lname"),
            F.col("age"),
        ...
        dfs_joined = self.dfs_employee.join(
            self.dfs_store, on=["store_id"], how="full_outer"
        ).select(
            self.dfs_employee.employee_id,
            self.dfs_employee["fname"],
            SF.col("lname"),
    ...
    def test_join_select_and_select_start(self):
        ...
        ).join(self.df_store, "store_id", "inner")

        dfs = self.dfs_employee.select(
            SF.col("fname"), SF.col("lname"), SF.col("age"), SF.col("store_id")
        ).join(self.dfs_store, "store_id", "inner")
    ...
    def test_branching_root_dataframes(self):
        ...
        """
        df_hydra_employees_only = self.df_employee.where(F.col("store_id") == F.lit(1))
        df_joined = (
            self.df_employee.where(F.col("store_id") == F.lit(2))
        ...
        dfs_hydra_employees_only = self.dfs_employee.where(SF.col("store_id") == SF.lit(1))
        dfs_joined = (
            self.dfs_employee.where(SF.col("store_id") == SF.lit(2))
    ...
    def test_basic_union(self):
        df_unioned = self.df_employee.select(F.col("employee_id"), F.col("age")).union(
            self.df_store.select(F.col("store_id"), F.col("num_sales"))
        )

        dfs_unioned = self.dfs_employee.select(SF.col("employee_id"), SF.col("age")).union(
            self.dfs_store.select(SF.col("store_id"), SF.col("num_sales"))
        )
    ...
    def test_union_with_join(self):
        ...
        df_unioned = df_joined.select(F.col("store_id"), F.col("store_name")).union(
            self.df_district.select(F.col("district_id"), F.col("district_name"))
        )
        ...
        dfs_unioned = dfs_joined.select(SF.col("store_id"), SF.col("store_name")).union(
            self.dfs_district.select(SF.col("district_id"), SF.col("district_name"))
        )
    ...
    def test_double_union_all(self):
        df_unioned = (
            self.df_employee.select(F.col("employee_id"), F.col("fname"))
            .unionAll(self.df_store.select(F.col("store_id"), F.col("store_name")))
            .unionAll(self.df_district.select(F.col("district_id"), F.col("district_name")))
        )

        dfs_unioned = (
            self.dfs_employee.select(SF.col("employee_id"), SF.col("fname"))
            .unionAll(self.dfs_store.select(SF.col("store_id"), SF.col("store_name")))
            .unionAll(self.dfs_district.select(SF.col("district_id"), SF.col("district_name")))
        )
    ...
    def test_order_by_default(self):
        df = (
            self.df_store.groupBy(F.col("district_id"))
            .agg(F.min("num_sales"))
            .orderBy(F.col("district_id"))
        )

        dfs = (
            self.dfs_store.groupBy(SF.col("district_id"))
    ...
    def test_intersect(self):
        df_employee_duplicate = self.df_employee.select(
            F.col("employee_id"), F.col("store_id")
        ).union(self.df_employee.select(F.col("employee_id"), F.col("store_id")))

        df_store_duplicate = self.df_store.select(F.col("store_id"), F.col("district_id")).union(
            self.df_store.select(F.col("store_id"), F.col("district_id"))
        )

        df = df_employee_duplicate.intersect(df_store_duplicate)

        dfs_employee_duplicate = self.dfs_employee.select(
            SF.col("employee_id"), SF.col("store_id")
        ).union(self.dfs_employee.select(SF.col("employee_id"), SF.col("store_id")))

        dfs_store_duplicate = self.dfs_store.select(
            SF.col("store_id"), SF.col("district_id")
        ).union(self.dfs_store.select(SF.col("store_id"), SF.col("district_id")))
    ...
    def test_distinct(self):
        df = self.df_employee.select(F.col("age")).distinct()

        dfs = self.dfs_employee.select(SF.col("age")).distinct()

        self.compare_spark_with_sqlglot(df, dfs)

    def test_union_distinct(self):
        df_unioned = (
            self.df_employee.select(F.col("employee_id"), F.col("age"))
            .union(self.df_employee.select(F.col("employee_id"), F.col("age")))
            .distinct()
        )

        dfs_unioned = (
            self.dfs_employee.select(SF.col("employee_id"), SF.col("age"))
            .union(self.dfs_employee.select(SF.col("employee_id"), SF.col("age")))
            .distinct()
        )
        self.compare_spark_with_sqlglot(df_unioned, dfs_unioned)

    def test_drop_duplicates_no_subset(self):
        df = self.df_employee.select("age").dropDuplicates()
        dfs = self.dfs_employee.select("age").dropDuplicates()
        self.compare_spark_with_sqlglot(df, dfs)

    def test_drop_duplicates_subset(self):
        df = self.df_employee.dropDuplicates(["age"])
        dfs = self.dfs_employee.dropDuplicates(["age"])
        self.compare_spark_with_sqlglot(df, dfs)
    ...
    def test_replace_basic(self):
        df = self.df_employee.select(F.col("age"), F.lit(37).alias("test_col")).replace(
            to_replace=37, value=100
        )

        dfs = self.dfs_employee.select(SF.col("age"), SF.lit(37).alias("test_col")).replace(
            to_replace=37, value=100
        )
    ...
    def test_replace_mapping(self):
        df = self.df_employee.select(F.col("age"), F.lit(37).alias("test_col")).replace({37: 100})
    ...
    def test_replace_na_func_basic(self):
        df = self.df_employee.select(F.col("age"), F.lit(37).alias("test_col")).na.replace(
            to_replace=37, value=100
        )

        dfs = self.dfs_employee.select(SF.col("age"), SF.lit(37).alias("test_col")).na.replace(
            to_replace=37, value=100
        )
    ...
    def test_with_column(self):
        df = self.df_employee.withColumn("test", F.col("age"))

        dfs = self.dfs_employee.withColumn("test", SF.col("age"))

        self.compare_spark_with_sqlglot(df, dfs)

    def test_with_column_existing_name(self):
        df = self.df_employee.withColumn("fname", F.lit("blah"))

        dfs = self.dfs_employee.withColumn("fname", SF.lit("blah"))
    ...
    def test_with_column_renamed(self):
        ...
        dfs = self.dfs_employee.withColumnRenamed("fname", "first_name")

        self.compare_spark_with_sqlglot(df, dfs)

    def test_with_column_renamed_double(self):
        df = self.df_employee.select(F.col("fname").alias("first_name")).withColumnRenamed(
            "first_name", "first_name_again"
        )

        dfs = self.dfs_employee.select(SF.col("fname").alias("first_name")).withColumnRenamed(
            "first_name", "first_name_again"
        )
    ...
    def test_drop_column_single(self):
        df = self.df_employee.select(F.col("fname"), F.col("lname"), F.col("age")).drop("age")

        dfs = self.dfs_employee.select(SF.col("fname"), SF.col("lname"), SF.col("age")).drop("age")

        self.compare_spark_with_sqlglot(df, dfs)

    def test_drop_column_reference_join(self):
        df_spark_employee_cols = self.df_employee.select(
            F.col("fname"), F.col("lname"), F.col("age"), F.col("store_id")
        )
        df_spark_store_cols = self.df_store.select(F.col("store_id"), F.col("store_name"))
        df = df_spark_employee_cols.join(df_spark_store_cols, on="store_id", how="inner").drop(
            df_spark_employee_cols.age,
        )

        df_sqlglot_employee_cols = self.dfs_employee.select(
            SF.col("fname"), SF.col("lname"), SF.col("age"), SF.col("store_id")
        )
        df_sqlglot_store_cols = self.dfs_store.select(SF.col("store_id"), SF.col("store_name"))
    ...
    def test_limit(self):
        df = self.df_employee.limit(1)

        dfs = self.dfs_employee.limit(1)

        self.compare_spark_with_sqlglot(df, dfs)

    def test_hint_broadcast_alias(self):
        df_joined = self.df_employee.join(
            self.df_store.alias("store").hint("broadcast", "store"),
            on=self.df_employee.store_id == self.df_store.store_id,
            how="inner",
        ).select(
            self.df_employee.employee_id,
        ...
        dfs_joined = self.dfs_employee.join(
            self.dfs_store.alias("store").hint("broadcast", "store"),
            on=self.dfs_employee.store_id == self.dfs_store.store_id,
            how="inner",
        ).select(
            self.dfs_employee.employee_id,
    ...
    def test_hint_broadcast_no_alias(self):
        ...
        self.assertIn("ResolvedHint (strategy=broadcast)", self.get_explain_plan(dfs))
        self.assertEqual(
            "'UnresolvedHint BROADCAST, ['store]", self.get_explain_plan(dfs).split("\n")[1]
        )

    def test_broadcast_func(self):
        df_joined = self.df_employee.join(
            F.broadcast(self.df_store),
            on=self.df_employee.store_id == self.df_store.store_id,
            how="inner",
        ).select(
            self.df_employee.employee_id,
    ...
    def test_repartition_by_num(self):
        ...
        """
        df = self.df_employee.repartition(63)
    ...
    def test_repartition_name_only(self):
        ...
        """
        df = self.df_employee.repartition("age")
    ...
    def test_repartition_num_and_multiple_names(self):
        ...
        We use the view here to help ensure the explain plans are similar enough to compare
        """
        df = self.df_employee.repartition(53, "age", "fname")
    ...
    def test_coalesce(self):
        df = self.df_employee.coalesce(1)
        dfs = self.dfs_employee.coalesce(1)
        df, dfs = self.compare_spark_with_sqlglot(df, dfs)
        spark_num_partitions = df.rdd.getNumPartitions()
        sqlglot_num_partitions = dfs.rdd.getNumPartitions()
    ...
    def test_cache_select(self):
        df_employee = (
            self.df_employee.groupBy("store_id")
            .agg(F.countDistinct("employee_id").alias("num_employees"))
            .cache()
        )
        df_joined = df_employee.join(self.df_store, on="store_id").select(
            self.df_store.store_id, df_employee.num_employees
        )
        dfs_employee = (
            self.dfs_employee.groupBy("store_id")
            .agg(SF.countDistinct("employee_id").alias("num_employees"))
            .cache()
        )
        dfs_joined = dfs_employee.join(self.dfs_store, on="store_id").select(
            self.dfs_store.store_id, dfs_employee.num_employees
        )
        self.compare_spark_with_sqlglot(df_joined, dfs_joined)
